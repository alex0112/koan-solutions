...
def max_profit(prices)
  ...
  future               = prices[index+1..-1]               ## Slice an array of all the prices following the minimum.
  highest_future_price = future.max                        ## From those future values, find the highest value.
  profit               = (index == prices.length - 1) ?
                           0 : highest_future_price - min  ## The maximum profit that could have been made, if one had bought at the lowest point
                                                           ## and sold at the highest future point.  If no profit was possible, return 0.
end

p max_profit([10, 7, 5, 8, 11, 9]) ## Returns 6
p max_profit([100, 9, 8, 78, 40])  ## Returns 70
p max_profit([0, 1, 2, 3, 4, 5])   ## 5
p max_profit([9, 8, 7, 6, 5, 4])   ## 0
p max_profit([1, 1, 1, 1])         ## 0
